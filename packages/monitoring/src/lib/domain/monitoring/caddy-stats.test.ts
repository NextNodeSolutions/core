...
describe('buildCaddyStatsQuery', () => {
	it('filters by project and the Caddy access logger, computing the three aggregates', () => {
		const query = buildCaddyStatsQuery('stylot')
		expect(query).toContain('nn_project:"stylot"')
		expect(query).toContain('"logger":"http.log.access"')
