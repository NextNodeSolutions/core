...
const { createProvider: mockCreateProvider } = await import("../src/providers/registry.js");
const { renderTemplate: mockRenderTemplate } = await import("../src/templates/renderer.js");

interface TestProps {
  name: string;
}

const testTemplate = (_props: TestProps) => null as unknown as React.ReactElement;
...
describe("createEmailManager (FR-5)", () => {
  ...
  it("creates an email manager with provider", async () => {
    ...
    expect(mockCreateProvider).toHaveBeenCalledWith("resend", {
      apiKey: "re_test",
    });
  });
...
describe("EmailManager.send() (FR-7, FR-9)", () => {
  ...
  it("sends email with template", async () => {
    ...
    expect(mockRenderTemplate).toHaveBeenCalledWith(testTemplate, { name: "John" }, undefined);
