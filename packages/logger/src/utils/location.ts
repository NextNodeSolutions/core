...
const isValidStackLine = (line: string): boolean =>
	typeof line === "string" && line.includes("at ") && !line.includes("node_modules");
...
const truncateStackLine = (line: string): string =>
	line.length > MAX_STACK_LINE_LENGTH ? line.slice(0, MAX_STACK_LINE_LENGTH) : line;
...
const extractFileInfo = (stackLine: string): { file: string; line: number } => {
	...
	for (const pattern of STACK_TRACE_PATTERNS.file) {
		...
		if (match?.[1] && match[2]) {
			...
			const file =
				lastSeparatorIndex !== -1 ? fullPath.slice(lastSeparatorIndex + 1) : fullPath;
