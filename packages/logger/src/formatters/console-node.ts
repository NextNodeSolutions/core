...
const colorize = (text: string, color: string): string => `${color}${text}${COLORS.reset}`;

const formatPropertyLines = (key: string, valueStr: string, prefix: string): string[] => {
...
const formatObjectDetails = (object: LogEntry["object"]): string[] => {
	...
	for (let i = 0; i < entries.length; i++) {
		...
		const prefix = isLast ? "└─" : "├─";
		const valueStr = typeof value === "object" ? safeStringify(value) : String(value);
...
export const formatForNode = (entry: LogEntry): string => {
	...
	components.push(colorize(level.toUpperCase().padEnd(5), LOG_LEVEL_COLORS[level]));
	...
	// Timestamp
	components.push(colorize(`[${formatTimeForDisplay(timestamp)}]`, COLORS.gray));

	// Message
	components.push(message);

	// Location and request ID (dimmed)
	components.push(colorize(`(${formatLocationForDisplay(location)}) [${requestId}]`, COLORS.dim));
