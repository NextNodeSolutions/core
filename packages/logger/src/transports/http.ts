 ...
 */
const RESTRICTED_HEADERS = new Set(["host", "content-length", "transfer-encoding"]);

export class HttpTransport implements Transport {
	private readonly config: Required<
		Pick<
			HttpTransportConfig,
			"endpoint" | "batchSize" | "flushInterval" | "timeout" | "maxRetries"
		>
	> &
		HttpTransportConfig;
	...
	private validateHeaders(headers?: Record<string, string>): void {
		...
		for (const key of Object.keys(headers)) {
			if (RESTRICTED_HEADERS.has(key.toLowerCase())) {
				throw new Error(`Header "${key}" is restricted and cannot be overridden`);
			}
		}
	}
 ...
 */
export const createHttpTransport = (config: HttpTransportConfig): HttpTransport =>
